      ...
      <tr>
        <td style="background-color:#f2f0e5;height:100px;vertical-align:top;border-bottom:2px solid gray;">
          <div style="float:left;width:50%;border-right:1px solid gray;height:90%;margin-top:5px;">
            ...
            <!-- 사람인원수 전달 -->
            <select id="combnum" name="manCount" style="background-color:white;width:100px;border-width:0px;margin-top:30px;"
          ...
          <!-- get으로 받아온 데이터 뿌리기 -->
            <label style="margin-left:10px;">장소:</label><label style="margin-left:10px;">${requestScope.audi}</label><br>
            <label style="margin-left:10px;">날짜/시간 : ${requestScope.day}</label><br>
            <label style="margin-left:10px;">가격: </label><label style="margin-left:10px;" id="price">14000</label>
        ...
        <tr style="text-align:center;">
          <td style="background-color:#fff6ea;height:400px;vertical-align: top;">
            <br><b><button style="border:1px solid gray; width:500px; background-color:#F0F0F0;height:30px;">S C R E E
           ...
           <c:forEach var="seatList" items="${seatList}" varStatus="st">
            ...
            <c:if test="${st.count < 11 }">
            <input type="checkbox" ${seperate } name="${alreadySeat}" ${open} value="${valueSep}"><label ${labelSep }>${st.count % 11 }</label>
            </c:if>
            <c:if test="${st.count >= 11 && st.count < 21 }">
            <input type="checkbox" ${seperate } name="${alreadySeat}" ${open} value="${valueSep}"><label ${labelSep }>${st.count % 11 + 1 }</label>
            </c:if>
            <c:if test="${st.count >= 21 && st.count < 31 }">
            <input type="checkbox" ${seperate } name="${alreadySeat}" ${open} value="${valueSep}"><label ${labelSep }>${st.count % 21  + 1}</label>
            </c:if>
            <c:if test="${st.count >= 31 && st.count < 41 }">
            <input type="checkbox" ${seperate } name="${alreadySeat}" ${open} value="${valueSep}"><label ${labelSep }>${st.count % 31  + 1}</label>
            </c:if>
            <c:if test="${st.count >= 41 && st.count < 51 }">
            <input type="checkbox" ${seperate } name="${alreadySeat}" ${open} value="${valueSep}"><label ${labelSep }>${st.count % 41  + 1}</label>
            </c:if>
            </c:forEach>
          </td>
        </tr>
